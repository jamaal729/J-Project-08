...
public class CheckoutController {
    ...
    String postCouponCode(Model model,
        ...
        if (!result.hasErrors()) {
            ...
            attributes.addFlashAttribute(
                    "flash", new FlashMessage("The Coupon code is accepted.", SUCCESS));
            return "redirect:shipping";
        } else {
            attributes.addFlashAttribute(
                    "flash", new FlashMessage("Invalid coupon code. If a code is entered it and must be between 5 and 10 characters", FAILURE));
    ...
    String checkoutConfirmation(Model model) {
        ...
        if (purchase != null) {
            ...
            model.addAttribute("creditCard", maskCardNumber(purchase.getCreditCardNumber()));
        } else {
            logger.error("No purchases Found!");
            return ("redirect:/error");
        }
    ...
    public void getFile(HttpServletResponse response) {
        ...
        try {
            ...
            if (purchase != null) {
                ...
                // TODO: Bugfix 4
                // Remove the billing address and all payment info from the email template
                 ctx.setVariable("billingAddress", purchase.getBillingAddress());
                 ctx.setVariable("creditCard", maskCardNumber(purchase.getCreditCardNumber()));
    ...
    public static String maskCardNumber(String creditCardNumber) {
        ...
        if (creditCardNumber == null)
            ...
            numberLength = creditCardNumber.length();

        char[] creditCardChar = new char[numberLength];
        for (int i = 0; i < numberLength; i++) {
            if ((i < numberLength - 4))
                creditCardChar[i] = '*';
            else
                creditCardChar[i] = creditCardNumber.charAt(i);
        }
        return new String(creditCardChar);
    }
}
